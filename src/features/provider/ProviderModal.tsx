...
export function ProviderModal({
  ...
  const handleModeChange = useCallback(
    (mode: ProviderConfig['mode']) => {
      ...
      if (mode === 'managed') {
        ...
        const nextProvider =
          managedProviders.find((item) => item.id === nextProviderId) ?? fallback ?? null
        onDraftConfigChange(
          normalizeProviderConfig({
            ...draftConfig,
            mode: 'managed',
            ...
            model: nextProvider?.models.includes(draftConfig.model.trim())
              ? draftConfig.model
              : (nextProvider?.defaultModel ?? draftConfig.model),
          }),
        )
        return
      }
    ...
    [
      ...
      draftConfig,
      managedProviders,
      normalizedDraftConfig.managedProviderId,
      onDraftConfigChange,
      resetTransientState,
    ],
  )

  const handleManagedProviderChange = useCallback(
    (providerId: string) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setSaveState({
        status: 'success',
        error: null,
        message: '配置已保存，后续生成和测试都会走同一条代理链路。',
      })
    } catch (error) {
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="接入配置">
      <div className="modal-card">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="eyebrow">接入配置</p>
            <h2 className="mt-2 font-display text-4xl leading-none">平台默认接入 / 自定义接入</h2>
            <p className="mt-3 max-w-xl text-sm leading-6 text-porcelain-100/[0.6]">
              管理员可以在后台发布公共 Provider
              作为平台默认接入；你也可以切到自定义接入，单独填写自己的云端基址、模型和 API Key。
            </p>
          </div>
        ...
        <div className="mt-6 rounded-[1.75rem] border border-signal-cyan/20 bg-signal-cyan/[0.055] p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-porcelain-100/[0.55]">
            生成和图生图都会走同一套服务端代理。公共 Provider 的 API URL 和 API Key
            由管理员托管，前台只能选择，不能查看。
          </p>
        </div>
        ...
        {!canUseManagedProviders || !canUseCustomProvider ? (
          <div className="mt-4 rounded-[1.5rem] border border-amber-400/20 bg-amber-400/[0.08] p-4 text-sm text-amber-100/80">
            {!canUseManagedProviders ? <p>当前账号未开放平台默认接入权限。</p> : null}
            {!canUseCustomProvider ? <p>当前账号未开放自定义接入权限。</p> : null}
          </div>
        ) : null}
        ...
        {normalizedDraftConfig.mode === 'managed' ? (
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <label className="field-block">
              <span className="field-label">平台默认 Provider</span>
              <select
            ...
            <div className="field-block">
              <label className="field-label" htmlFor="managed-provider-model">
                Model
              </label>
            ...
            <div className="md:col-span-2 rounded-[1.5rem] border border-porcelain-50/10 bg-porcelain-50/[0.03] p-4 text-xs leading-6 text-porcelain-100/[0.62]">
              ...
              <p className="mt-2">
                ...
                {managedProviders.length
                  ? `${managedProviders.length} 个`
                  : '暂无可用公共 Provider，请联系管理员配置。'}
              </p>
              {selectedManagedProvider?.description ? (
                <p className="mt-2">{selectedManagedProvider.description}</p>
              ) : null}
            </div>
          </div>
        ) : (
        ...
        <div className="mt-6 rounded-[1.75rem] border border-porcelain-50/10 bg-porcelain-50/[0.035] p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-xs leading-5 text-porcelain-100/[0.55]">
                测试连接只针对已保存配置执行。如果你刚修改了公共 Provider
                选择或自定义参数，请先保存。
              </p>
            </div>
            <button
              ...
              aria-describedby={
                connectionTest.status === 'idle' ? undefined : 'provider-connection-test-result'
              }
            >
              {isTestingConnection ? (
          ...
          {connectionTest.status !== 'idle' && (
            ...
            >
              <div className="flex items-start gap-2 font-semibold">
                ...
                {connectionTest.status === 'error' ? (
                  <div className="min-w-0 flex-1">
                    <ErrorNotice error={connectionTest.error ?? '测试连接失败。'} compact />
                  </div>
                ) : (
              ...
              {connectionTest.requestUrl && (
                <p className="mt-2 break-all font-mono text-[11px] leading-5 opacity-75">
                  测试地址：{connectionTest.requestUrl}
                </p>
              )}
            </div>
          )}
        ...
        <div className="mt-4 rounded-[1.75rem] border border-porcelain-50/10 bg-porcelain-50/[0.03] p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-porcelain-100/[0.65]">
            当前接入方式：{normalizedDraftConfig.mode === 'managed' ? '平台默认' : '自定义接入'}
          </p>
          <p className="mt-2 text-xs leading-5 text-porcelain-100/[0.65]">
        ...
        <div className="mt-6 rounded-[1.75rem] border border-signal-cyan/20 bg-signal-cyan/[0.055] p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-porcelain-100/[0.55]">
            公共 Provider 的敏感信息由后台托管；自定义 Provider 的 API Key
            会按当前账号配置保存到后端，不会长期落在浏览器本地。
          </p>
        </div>
        ...
        {saveState.status !== 'idle' && (
          ...
          >
            <div className="flex items-start gap-2 font-semibold">
              ...
              {saveState.status === 'error' ? (
                <div className="min-w-0 flex-1">
                  <ErrorNotice error={saveState.error ?? '保存失败。'} compact />
                </div>
              ) : (
