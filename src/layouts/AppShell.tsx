      ...
      <header className="fixed left-0 right-0 top-0 z-[60] border-b border-porcelain-50/10 bg-ink-950/[0.7] backdrop-blur-2xl">
        <div className="mx-auto flex w-full max-w-screen-xl flex-wrap items-center justify-between gap-4 px-4 py-4 md:px-8">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/studio">
              工作台
            </NavLink>
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/templates">
              模板
            </NavLink>
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/works">
              作品
            </NavLink>
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/tasks">
              任务
            </NavLink>
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/account">
              账户
            </NavLink>
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/providers">
              配置中心
            </NavLink>
            <NavLink className={({ isActive }) => navLinkClassName(isActive)} to="/app/billing">
              支付
            </NavLink>
            {canAccessAdmin ? (
              <NavLink
            ...
            <button
              className="rounded-full border border-porcelain-50/10 bg-ink-950/[0.65] px-4 py-2 text-sm font-semibold text-porcelain-50 transition hover:border-signal-coral/50 hover:text-signal-coral"
