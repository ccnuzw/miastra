...
function resolveTaskWork(task: GenerationTaskRecord, indexes: WorkIndexes) {
  ...
  const snapshotCandidates = [task.result?.snapshotId, task.payload.snapshotId].filter(Boolean) as string[]
...
function buildTaskBatchView(params: {
  ...
  const slots = [...slotMap.entries()]
    .map(([slotId, attempts]) => {
      ...
      return {
        slotId,
        title: latestTask.payload.title || `任务 ${latestTask.drawIndex ?? latestTask.id.slice(0, 8)}`,
        latestTask,
  ...
  const stats = slots.reduce((accumulator, slot) => {
    ...
    else if (slot.latestTask.status === 'pending' || slot.latestTask.status === 'queued') accumulator.pending += 1
  ...
  return {
    ...
    updatedAt: Math.max(...slots.map((slot) => toTimestamp(slot.latestTask.updatedAt)), params.createdAt),
...
export function TasksPage() {
  ...
  const batchViews = useMemo(() => {
    ...
    const items: TaskBatchView[] = batches.map((batch) => buildTaskBatchView({
      id: batch.id,
      title: batch.title,
    ...
    for (const [batchId, batchTasks] of batchTaskMap.entries()) {
      ...
      items.push(buildTaskBatchView({
        id: batchId,
        title: batchId,
        tasks: batchTasks,
        createdAt: Math.min(...batchTasks.map((task) => toTimestamp(task.createdAt))),
        snapshotId: batchTasks[0]?.payload.draw?.batchSnapshotId ?? batchTasks[0]?.payload.snapshotId,
  ...
  useEffect(() => {
    ...
    setExpandedBatchIds((current) => {
      ...
      const preferred = batchViews.find((batch) => batch.status === 'running' || batch.stats.failed > 0) ?? batchViews[0]
  ...
  useEffect(() => {
    ...
    const exists = batchViews.some((batch) => batch.slots.some((slot) => slot.slotId === expandedSlotId))
    if (!exists) setExpandedSlotId('')
  }, [batchViews, expandedSlotId])

  const overallStats = useMemo(() => batchViews.reduce((accumulator, batch) => {
  ...
  const overallSuccessRate = overallStats.slotCount > 0
  ...
  const workIndexes = useMemo<WorkIndexes>(() => {
    ...
    for (const work of works) {
      ...
      if (work.snapshotId && !bySnapshotId.has(work.snapshotId)) bySnapshotId.set(work.snapshotId, work)
  ...
  function toggleBatch(batchId: string) {
    setExpandedBatchIds((current) => current.includes(batchId)
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-screen-xl px-4 pb-10 pt-32 md:px-8">
      <section className="panel-shell w-full">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <article className="progress-card">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">任务槽位</p>
            <p className="mt-3 text-3xl font-semibold text-porcelain-50">{overallStats.slotCount}</p>
            <p className="mt-2 text-sm text-porcelain-100/55">按批次去重后的可追踪任务项</p>
          </article>
          <article className="progress-card">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">进行中</p>
          ...
          <article className="progress-card">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">待恢复</p>
          ...
          <article className="progress-card">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">成功率</p>
        ...
        {actionMessage ? <p className="mt-6 rounded-[1.3rem] border border-signal-cyan/25 bg-signal-cyan/[0.08] px-4 py-3 text-sm text-signal-cyan">{actionMessage}</p> : null}

        <div className="mt-8 grid gap-5">
          {batchViews.map((batch) => {
            const expanded = expandedBatchIds.includes(batch.id)
            const completedCount = batch.stats.succeeded + batch.stats.failed + batch.stats.cancelled
            const completionRate = batch.total > 0 ? Math.round((completedCount / batch.total) * 100) : 0
            const successRate = batch.total > 0 ? Math.round((batch.stats.succeeded / batch.total) * 100) : 0
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusTone(batch.status)}`}>{statusLabel(batch.status)}</span>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-porcelain-100/52">
                      <span className="rounded-full border border-porcelain-50/10 px-3 py-1">总数 {batch.total}</span>
                      <span className="rounded-full border border-signal-cyan/20 bg-signal-cyan/[0.08] px-3 py-1 text-signal-cyan">成功 {batch.stats.succeeded}</span>
                      <span className="rounded-full border border-signal-coral/20 bg-signal-coral/10 px-3 py-1 text-signal-coral">失败 {batch.stats.failed}</span>
                      <span className="rounded-full border border-porcelain-50/10 px-3 py-1">进行中 {batch.stats.running + batch.stats.pending}</span>
                      <span className="rounded-full border border-porcelain-50/10 px-3 py-1">取消 {batch.stats.cancelled}</span>
                      <span className="rounded-full border border-porcelain-50/10 px-3 py-1">重试链路 {batch.stats.retrying}</span>
                    </div>
                    ...
                    <div className="mt-4">
                      <div className="flex flex-wrap items-center gap-3 text-xs text-porcelain-100/52">
                        ...
                        <span>结果入库 {batch.slots.filter((slot) => resolveTaskWork(slot.latestTask, workIndexes)).length}</span>
                      </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button type="button" className="rounded-full border border-porcelain-50/10 px-4 py-2 text-sm" onClick={() => toggleBatch(batch.id)}>{expanded ? '收起批次' : '展开批次'}</button>
                  </div>
                </div>

                {expanded ? (
                  <div className="mt-5 grid gap-4">
                    {batch.slots.length ? batch.slots.map((slot) => {
                      ...
                      const batchSnapshotId = task.payload.draw?.batchSnapshotId ?? batch.snapshotId ?? '—'
                      const resultSnapshotId = task.result?.snapshotId ?? resultWork?.snapshotId ?? '—'
                      const generationSnapshotId = readGenerationSnapshotId(resultWork?.generationSnapshot) ?? readGenerationSnapshotId(task.result?.generationSnapshot) ?? '—'
                      ...
                      return (
                        <article key={slot.slotId} className="rounded-[1.5rem] border border-porcelain-50/10 bg-ink-950/[0.45] p-4">
                          <div className="flex flex-wrap items-start justify-between gap-4">
                            <div>
                              <div className="flex flex-wrap items-center gap-3">
                                <h3 className="text-lg font-semibold text-porcelain-50">{slot.title}</h3>
                                <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusTone(task.status)}`}>{statusLabel(task.status)}</span>
                                {slot.attempts.length > 1 ? <span className="status-pill">尝试 {slot.attempts.length}</span> : null}
                              ...
                              <div className="mt-3 flex flex-wrap gap-4 text-xs text-porcelain-100/45">
                                ...
                                <span>批次位序：{task.drawIndex === undefined ? '—' : task.drawIndex + 1}</span>
                          ...
                          {isExpanded ? (
                            <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_320px]">
                              <div className="space-y-4">
                                <div className="grid gap-3 rounded-[1.35rem] border border-porcelain-50/10 bg-ink-950/[0.45] p-4 md:grid-cols-2">
                                  <div>
                                    ...
                                    <div className="mt-3 grid gap-2 text-sm text-porcelain-100/72">
                                      ...
                                      <p>批次序号：{task.drawIndex === undefined ? '—' : task.drawIndex + 1}</p>
                                      <p>变体：{task.variation ?? task.payload.draw?.variation ?? '—'}</p>
                                    </div>
                                  </div>
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">快照与结果</p>
                                    <div className="mt-3 grid gap-2 text-sm text-porcelain-100/72">
                                ...
                                <div className="grid gap-3 rounded-[1.35rem] border border-porcelain-50/10 bg-ink-950/[0.45] p-4 md:grid-cols-2">
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">请求信息</p>
                                    <div className="mt-3 grid gap-2 text-sm text-porcelain-100/72">
                                  ...
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">结果信息</p>
                                    <div className="mt-3 grid gap-2 text-sm text-porcelain-100/72">
                                      <p>结果标题：{resultWork?.title ?? task.result?.title ?? task.payload.title ?? '—'}</p>
                                      <p>结果模型：{resultWork?.providerModel ?? task.result?.providerModel ?? task.payload.model}</p>
                                      <p>参考图数量：{task.payload.referenceImages?.length ?? 0}</p>
                                      <p>模式来源：{modeLabel(task.result?.mode ?? resultWork?.mode ?? task.payload.mode)}</p>
                                      <p>作品快照来源：{resultWork ? 'works' : task.result ? 'task-result' : '—'}</p>
                                      <p>作品标签：{resultWork?.tags?.length ? resultWork.tags.join(' / ') : '—'}</p>
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div className="rounded-[1.35rem] border border-porcelain-50/10 bg-ink-950/[0.45] p-4">
                                  <p className="text-xs font-semibold uppercase tracking-[0.24em] text-porcelain-100/40">尝试历史</p>
                                  <div className="mt-3 grid gap-3">
                                    {slot.attempts.map((attempt) => (
                                      <div key={attempt.id} className="rounded-2xl border border-porcelain-50/10 bg-porcelain-50/[0.035] p-3">
                                        <div className="flex flex-wrap items-center justify-between gap-3">
                                          <div className="flex flex-wrap items-center gap-2">
                                            <strong className="text-sm text-porcelain-50">第 {attempt.retryAttempt + 1} 次</strong>
                                            <span className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${statusTone(attempt.status)}`}>{statusLabel(attempt.status)}</span>
                                          </div>
                                          <span className="text-xs text-porcelain-100/45">{formatDateTime(attempt.updatedAt)}</span>
                                        </div>
                                        ...
                                        {attempt.errorMessage ? <ErrorNotice error={attempt.errorMessage} compact className="mt-3" /> : null}
                                      </div>
                                    ))}
                                  </div>
                      ...
                      <div className="rounded-[1.5rem] border border-dashed border-porcelain-50/15 bg-porcelain-50/[0.03] px-4 py-6 text-sm text-porcelain-100/55">该批次暂时没有可展示的任务记录。</div>
                    )}
                  </div>
                ) : null}
              </article>
          ...
          {!loading && batchViews.length === 0 ? <div className="progress-card text-sm text-porcelain-100/60">当前还没有生成任务。</div> : null}
        </div>
      </section>
    </main>
  )
